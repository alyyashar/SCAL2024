...
from soltest import wrapper
from soltest.exceptions import (
    DownloadError,
...
from soltest.utils.lock import get_process_lock
...
LOGGER = logging.getLogger("soltest")
...
def get_soltest_install_folder(soltest_binary_path: Union[Path, str] = None) -> Path:
    """
    Return the directory where `py-solc-x` stores installed `solc` binaries.

    By default, this is `~/.soltest`

    Arguments
    ---------
    soltest_binary_path : Path | str, optional
    ...
    if _get_target_os() == _get_os_name():
        ...
        elif soltest_binary_path is not None:
            return Path(soltest_binary_path)
        else:
            path = Path.home().joinpath(".soltest")
            path.mkdir(exist_ok=True)
            return path
    else:
        path = Path.home().joinpath(f".soltest-{_get_target_os()}")
        path.mkdir(exist_ok=True)
        return path
...
def import_installed_solc(soltest_binary_path: Union[Path, str] = None) -> List[Version]:
    """
    ...
    for path in path_list:
        ...
        copy_path = get_soltest_install_folder(soltest_binary_path).joinpath(f"solc-v{version}")
        if _get_os_name() == "windows":
            copy_path.mkdir()
...
def get_executable(
    version: Union[str, Version] = None, soltest_binary_path: Union[Path, str] = None
) -> Path:
    """
    ...
    if not version:
        if not _default_solc_binary:
            raise SolcNotInstalled(
                "Solc is not installed. Call soltest.get_installable_solc_versions()"
                " to view for available versions and soltest.install_solc() to install."
            )
        return _default_solc_binary

    version = _convert_and_validate_version(version)
    solc_bin = get_soltest_install_folder(soltest_binary_path).joinpath(f"solc-v{version}")
    if _get_target_os() == "windows":
        solc_bin = solc_bin.joinpath("solc.exe")
    if not solc_bin.exists():
        raise SolcNotInstalled(
            f"solc {version} has not been installed."
            f" Use soltest.install_solc('{version}') to install."
        )
    return solc_bin


def set_solc_version(
    version: Union[str, Version], silent: bool = False, soltest_binary_path: Union[Path, str] = None
) -> None:
    """
    ...
    _default_solc_binary = get_executable(version, soltest_binary_path)
    if not silent:
...
def set_solc_version_pragma(
    ...
    if version is None:
        raise SolcNotInstalled(
            ...
            f" Use soltest.install_solc_version_pragma('{version}') to install."
        )
...
def install_solc_pragma(
    ...
    soltest_binary_path: Union[Path, str] = None,
) -> Version:
    """
    ...
    if install:
        install_solc(version, show_progress=show_progress, soltest_binary_path=soltest_binary_path)
...
def get_installed_solc_versions(soltest_binary_path: Union[Path, str] = None) -> List[Version]:
    """
    ...
    """
    install_path = get_soltest_install_folder(soltest_binary_path)
...
def install_solc(
    ...
    with process_lock:
        if _check_for_installed_version(version, soltest_binary_path):
            path = get_soltest_install_folder(soltest_binary_path).joinpath(f"solc-v{version}")
        ...
        if target_os == "linux":
            _install_solc_unix(version, filename, show_progress, soltest_binary_path)
        elif target_os == "macosx":
            _install_solc_unix(version, filename, show_progress, soltest_binary_path)
        elif target_os == "windows":
            _install_solc_windows(version, filename, show_progress, soltest_binary_path)

        try:
            _validate_installation(version, soltest_binary_path)
            ...
            if target_os != "windows" and target_os == this_os:
                exc.args = (
                    ...
                    f"source code using `soltest.compile_solc('{version}')`.",
                )
            raise exc
...
def compile_solc(
    version: Version, show_progress: bool = False, soltest_binary_path: Union[Path, str] = None
) -> Version:
    """
    ...
    with process_lock:
        ...
        install_path = get_soltest_install_folder(soltest_binary_path).joinpath(f"solc-v{version}")
...
def _check_for_installed_version(
    version: Version, soltest_binary_path: Union[Path, str] = None
) -> bool:
    path = get_soltest_install_folder(soltest_binary_path).joinpath(f"solc-v{version}")
    return path.exists()
...
def _get_temp_folder() -> Path:
    path = Path(tempfile.gettempdir()).joinpath(f"soltest-tmp-{os.getpid()}")
    if path.exists():
...
def _install_solc_unix(
    version: Version, filename: str, show_progress: bool, soltest_binary_path: Union[Path, str, None]
) -> None:
...
def _validate_installation(version: Version, soltest_binary_path: Union[Path, str, None]) -> None:
    if _get_target_os() != _get_os_name():
        return
    binary_path = get_executable(version, soltest_binary_path)
    try:
...
try:
    ...
    # if not available, use the most recent soltest installed version
...
if __name__ == "__main__":
    ...
    argument_parser.add_argument("--soltest-binary-path", default=None)
    args = argument_parser.parse_args()
    install_solc(args.version, soltest_binary_path=args.soltest_binary_path)
