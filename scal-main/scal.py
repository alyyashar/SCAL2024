...
import runner.docker_api as docker_api
import runner.cli as cli
import runner.config as config
import runner.logging as log
import runner.colors as col
import runner.parsing as parsing
import runner.output_parser.SarifHolder as SarifHolder


def process_datasets(logqueue, settings):
    if 'contract' not in settings or not settings['contract']:
        return
    if 'all' in settings['contract']:
        settings['contract'] = config.DATASET_CHOICES

    for contract in settings['contract']:
        base_name = contract.split('/')[0]
        ...
        if config.is_remote_info(base_path):
            ...
            if os.path.isdir(base_path): # locally installed
                log.message(logqueue, f"Using remote contract [{base_path} <- {url}]")
            elif os.path.isdir(global_path): # globally installed
                log.message(logqueue, f"Using remote contract [{global_path} <- {url}]")
                base_path = global_path
            else: # local copy does not exist; we need to clone it
                time.sleep(1)
                answer = input(f"{base_name} is a remote contract. Do you want to create a local copy? [Y/n] ").strip()
                if answer.lower() in ['yes', 'y', '']:
                    print(f"Cloning remote contract [{base_path} <- {url}]... ", flush=True, end='')
                    git.Repo.clone_from(url, base_path)
                    print("Done.", flush=True)
                else:
                    log.message(logqueue, col.error(f"ABORTING: cannot proceed without local copy of remote contract {base_name}"))
                    sys.exit(1)

            if contract == base_name:  # basename included
                settings["file"].append(base_path)
            if contract != base_name and base_name not in settings["contract"]:
                subset_name = contract.split('/')[1]
...
def collect_tasks(files_to_analyze, tools_to_use, bounce):
    tasks = []
    for file,id in files_to_analyze:
        for tool,tool_folder in tools_to_use:
            results_folder = os.path.join(tool_folder, id)
            os.makedirs(results_folder, exist_ok=True)
            if bounce and os.path.exists(os.path.join(results_folder,'result.json')):
                continue
...
def scal(settings):
    ...
    log.start(logqueue, settings["execution_name"], append=settings["bounce"])
    try:
        log.message(logqueue, col.success("Welcome to SCAL, The Automated Smart Contract Auditing Framework!"), f"Arguments passed: {sys.argv}")
        ...
        tasks            = collect_tasks(files_to_analyze, tools_to_use, settings["bounce"])
        ...
        for task in tasks:
            taskqueue.put(task)
        for _ in range(settings["operations"]):
            taskqueue.put(None)

        # set up and start analyzers
        shared = (logqueue, taskqueue, sarifqueue, tasks_started, tasks_completed, total_time)
        constants = (settings["import_path"], settings["output_version"], settings["operations"], len(tasks))
        analyzers = [ mp.Process(target=analyzer, args=shared+constants) for _ in range(settings["operations"]) ]
        for a in analyzers:
            a.start()
