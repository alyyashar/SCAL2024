...
import runner.solidity as solidity
...
def run(logqueue, file, tool, results_folder):
    ...
    shutil.copytree(os.path.join(config.TOOLS_CONFIG_PATH,tool["name"]), working_bin_dir)
    ...
    try:
        scal_json = os.path.join(results_folder, "scal.json")
        with open(scal_json, 'w') as f:
            json.dump(results, f, indent=2)
    except Exception as err:
        log.message(logqueue, col.error(f"{tool['name']}, {file}: Failing to write {scal_json}.\n{err}"))
